...
// Pregunta de contraseña con máscara (@inquirer/password). Sin TTY
// (CI/planificador) cae en ask() sin máscara.
const askPassword = async (question) => {
  if (isInteractiveTTY()) {
    try {
      const password = await loadPrompt('@inquirer/password');
      const answer = await password({ message: question, mask: '*' });
      restoreReadline();
      return answer;
    } catch (err) {
      restoreReadline();
...
module.exports = { ask, askPassword, askConfirm, askSelect, askCheckbox, ALL, restoreReadline, close };
