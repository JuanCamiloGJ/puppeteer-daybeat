...
console.log(`\n\nTotal de registros encontrados: ${existingDates.length}`);
  ...
  // Preguntar si se desea incluir información de Clockify. Si no hay key
  // configurada y quiere incluirla, se ofrece el flujo guiado de conexión.
  let useClockify = false;
  if (clockify.isConfigured()) {
    useClockify = await prompt.askConfirm('¿Desea incluir información de Clockify (entradas con horario) en los reportes?');
  } else if (await prompt.askConfirm('¿Desea incluir información de Clockify en los reportes? (todavía no hay API key configurada)')) {
    useClockify = await connectClockifyFlow();
  }
  ...
  console.log(`Incluir info de Clockify: ${useClockify ? 'Sí' : 'No'}`);
  ...
  for (let i = 0; i < missingDays.length; i++) {
    ...
    try {
      ...
      // Consultar actividad de Clockify para el día (se suma al contexto)
      if (useClockify) {
        console.log('  Consultando actividad en Clockify...');
        try {
          const data = await clockify.getDailyActivity(day);
          if (data.entries.length > 0) {
            const clockifyContext = clockify.formatActivityForReport(data);
            extraContext = extraContext
              ? `${extraContext}\n\n${clockifyContext}`
              : clockifyContext;
          }
        } catch (err) {
          console.log(`  ✗ Error consultando Clockify para ${day}: ${err.message}`);
        }
      }
