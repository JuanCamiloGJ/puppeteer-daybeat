...
// Busca un elemento del DOM con un callback y devuelve un ElementHandle REAL
// o null. A diferencia de `evaluateHandle` + `if (handle)` — que revienta con
// TypeError intermitente ("Cannot read properties of undefined") cuando el
// frame aún no renderizó el menú y el handle envuelve `null` (un JSHandle
// vacío es truthy) — valida con asElement() y hace dispose del handle vacío.
const findElementHandle = async (frame, fn, ...args) => {
  const handle = await frame.evaluateHandle(fn, ...args);
...
const getExistingRanges = async (frameTree, page, dateStr, currentUser = null, catValue = null, transValue = null) => {
  try {
    ...
    console.log('  [STAGE] getExistingRanges: yendo al detalle del item...');
    await navigateFrameRobust(page, async (ft) => {
    ...
    console.log('  [STAGE] getExistingRanges: leyendo transacciones del día...');
    const rows = await parseTransactionTable(frameTree, targetDate, currentUser);
    console.log(`  [STAGE] getExistingRanges: ${rows.length} transacciones del día encontradas`);

    // Volver al formulario de creación
    console.log('  [STAGE] getExistingRanges: volviendo al formulario de creación...');
    await navigateFrameRobust(page, async (ft) => {
    ...
    console.log('  [STAGE] getExistingRanges: formulario listo');
...
// Asigna un valor a un campo del formulario reemplazando el contenido actual
// (a diferencia de frame.type, que agrega texto y depende de eventos de teclado
// con mapeos frágiles para Unicode/newlines). Dispara input+change igual que el
// patrón de updateTransaction.
const setFieldValue = async (frame, selector, value) => {
  await frame.$eval(selector, (el, v) => {
    el.value = v;
