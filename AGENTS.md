...
After selecting category and transaction type, the user chooses one of three modes:
...
2. **Auto fake**: Uses commits from the last 7 days (filtered by author) to generate a structured summary. Reuses the last-used schedule from `.daybeat-history.json` (defaults to 0730-1630).
3. **Manual**: Original interactive flow — prompts for each field.
...
5. For each missing day:
   ...
   - If no commits that day, uses commits from last 7 days before that date
   - Generates title and detail automatically
