...
| **2. Varios bloques según actividad** | Divide el día en hasta 4 bloques contiguos según la hora de la actividad (commits y, en modo 5, comentarios/worklogs de Jira). La suma de los bloques es exactamente el mismo total del bloque único (nunca horas incompletas). Genera un resumen por bloque y registra una transacción por bloque. Requiere al menos 2 actividades con horario; si no, usa un solo bloque. El registro masivo siempre usa un solo bloque por día. Con esta opción la confirmación del bloque único se difiere: primero se muestra la propuesta de bloques, y la pregunta de "¿Desea continuar?" solo aparece si se rechazan los bloques o no hay actividad suficiente. |
